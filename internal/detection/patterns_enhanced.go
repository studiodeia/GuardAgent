...
type PatternDetector struct {
	patterns    map[common.ThreatType][]*CompiledPattern
	mlModels    map[common.ThreatType]MLModel
...
var enhancedBrazilianPatterns = map[common.ThreatType][]PatternConfig{
	common.ThreatPII: {
		{
			Name:        "cpf",
...
func NewEnhancedPatternDetector() *PatternDetector {
	pd := &PatternDetector{
		patterns:    make(map[common.ThreatType][]*CompiledPattern),
		mlModels:    make(map[common.ThreatType]MLModel),
...
func (pd *PatternDetector) DetectPII(ctx context.Context, text string) ([]PIIMatch, error) {
	...
	if !pd.bloomFilter.MightContain([]byte(normalized)) {
		...
		matches = pd.fastRegexScan(normalized, common.ThreatPII)
	} else {
		// Full detection pipeline
		matches = pd.fullDetectionPipeline(ctx, normalized, common.ThreatPII)
	}
...
func (pd *PatternDetector) fastRegexScan(text string, threatType common.ThreatType) []PIIMatch {
	var matches []PIIMatch
...
func (pd *PatternDetector) fullDetectionPipeline(ctx context.Context, text string, threatType common.ThreatType) []PIIMatch {
	var matches []PIIMatch
